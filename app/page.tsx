...
import { useReveal } from "../lib/reveal"; // <- relative path; no tsconfig paths needed

const COMPANY_NAME = "Rabia AlBadia Contracting Co.";
const CONTACT_EMAIL = "info@rbc-sa.com";
const CONTACT_PHONE = "+966539560033"; // E.164 format
        ...
        <div className="flex h-14 items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <a
              ...
              className="hidden rounded-lg border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:border-gray-500 md:inline-block"
            >
              Email
            </a>
            <a
  ...
  return (
    <section className="relative overflow-hidden border-b border-gray-200">
      ...
      <Container>
        <div
          ref={ref}
          className="relative grid grid-cols-1 items-center gap-10 py-16 md:grid-cols-2 md:py-24 reveal"
        >
          <div>
            ...
            <p className="mt-4 max-w-xl text-pretty text-gray-600">
              We deliver end-to-end metal works for landmark builds—shop drawings, precise
              fabrication, and clean installs. Riyadh-based, KSA-wide execution.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a
                href={WHATSAPP_LINK}
                className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 hover-lift"
              >
                Start your project <ArrowRight className="h-4 w-4" />
              </a>
              <a
                href={`mailto:${CONTACT_EMAIL}`}
                className="inline-flex items-center gap-2 rounded-xl border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:border-gray-500 hover-lift"
              >
  ...
  return (
    <section>
      <Container>
        <div
          ref={ref}
          className="reveal grid grid-cols-2 gap-4 py-10 text-xs text-gray-600 md:grid-cols-4"
        >
...
function Services() {
  ...
  return (
    <section id="services" className="border-t border-gray-200">
      <Container>
        <div className="reveal py-14" ref={ref}>
          <h2 className="text-2xl font-semibold">Services</h2>
          <p className="mt-2 max-w-2xl text-sm text-gray-600">
            We turn drawings into delivered metalwork—clear scope, clear timeline, clean finish.
          </p>
          <p className="mt-2 text-sm">
            ...
            <a href="/contact" className="text-blue-600 hover:text-blue-700 underline decoration-blue-200 underline-offset-2 ml-1">get in touch for a quote</a>.
          </p>
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
            {items.map((it) => (
              <a
                ...
                className="rounded-2xl border border-gray-200 bg-white p-5 hover-lift block transition-all duration-200 hover:border-blue-300 hover:shadow-lg"
              >
...
function FeaturedProjects() {
  ...
  useEffect(() => {
    fetch("/api/projects?featured=1")
      .then((res) => res.json())
      .then((data) => setFeatured(data.projects?.slice(0, 2) || []))
        ...
        <article key={project.id} className="overflow-hidden rounded-2xl border border-gray-200 bg-white hover-lift">
          <a href={`/projects/${project.slug}`}>
            ...
            <div className="p-5">
              ...
              <p className="mt-2 text-sm text-gray-600">Featured Project - Latest work and details inside</p>
            </div>
          </a>
        </article>
      ))}
...
function CTA() {
  ...
  return (
    <section id="contact" className="border-t border-gray-200">
      <Container>
        <div className="reveal py-16 text-center" ref={ref}>
          <h2 className="text-balance text-3xl font-semibold">Ready to move from drawing to delivered?</h2>
          <p className="mx-auto mt-2 max-w-xl text-gray-600">
            Send your scope for a fast, on-spec quote. We respond same-day during working hours.
          </p>
